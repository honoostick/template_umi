...
const Tags = ({ className = '', data = [], theme = 'gray', ...rest }) => (
  <ul {...rest} className={`${styles.tags} ${className}`}>
    {data
      .filter(tag => !!tag)
      .map(tag => (
        <li key={uuid()} className={`${styles.tag} ${styles[theme]}`}>{tag}</li>
      ))}
  </ul>
);
...
const CompanyExhibitItem = ({
  ...
  <li className={styles.item} onClick={() => window.open(`/brand/${enterpriseId}`)}>
    <FlexRow>
      <Avatar
        style={{ flex: 'none' }}
        size={145}
      ...
      <section className={styles.detail}>
        <div className={styles.name}>{enterpriseName}</div>
        <Tags
          className="GMLL-1"
          data={labelList.map(item => item.tagName)}
          theme="blue"
          style={{ fontSize: 12 }}
        />
        <article className="GMLL-3">{companyProfile}</article>
...
export default ({ data = [], ...rest }) => (
  <div className={styles.container}>
    <ul>
      {data.map(item => (
        <CompanyExhibitItem key={uuid()} {...item} {...rest} />
      ))}
    </ul>
  </div>
);
