...
export default class CheckAbleList extends React.PureComponent {
  ...
  checked = item => {
    const { onChange = () => { }, multiple = false, data } = this.props;
    if ((!multiple && item && item.checked) || !data || data.length === 0) {
      // 单选 不支持重复点击
      return;
    }
    const values = [];
    ...
    const newData = data.map((it, index) => {
      ...
      if (!multiple) {
        ...
        } else if (item && newItem.value === item.value) {
          newItem.checked = true;
        }
    ...
    onChange(multiple ? values : values[0], newData, firstIndex);
  }
    ...
    return (
      ...
      >
        ...
        >
          {
            data.map((item, index) => {
              const isChecked = index === checkIndex;
                ...
                return (
                  <Fragment key={`${item.value}_frag`}>
                    <li
                      key={`${item.value}`}
                      style={{ ...itemStyle }}
                      className={`${styles.item} ${isChecked ? (checkedClassName || styles.checked) : ''} ${itemClassName}`}
                      onClick={() => this.checked(item)}
                    >
                      {item.text}
                    </li>
                    {
              ...
              return <li
                ...
                style={{ ...itemStyle }}
                className={`${styles.item} ${isChecked ? (checkedClassName || styles.checked) : ''} ${itemClassName}`}
                onClick={() => this.checked(item, props)}>{item.text}</li>;
            })
          }
        </ul>
      </div>
